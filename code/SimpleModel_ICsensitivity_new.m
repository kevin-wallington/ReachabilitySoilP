...
x1_init_vec = (160:5:240); % 21 ICs
x2_init_vec = (1313.6:32.84:1917.4); % 21 ICs
num_sets = length(x1_init_vec)*length(x2_init_vec);
...
hitmap = zeros(length(x2_init_vec),length(x1_init_vec));
...
%%%%% debug end
for zz=1:num_sets
    x1_id = (floor((zz-1)/length(x2_init_vec))+1);
    x1_init_temp = x1_init_vec(x1_id); 
    x2_id =  mod(zz,length(x2_init_vec));
    if(x2_id==0); x2_id= length(x2_init_vec); end
    x2_init_temp = x2_init_vec(x2_id); 
...
%     x2_plot(zz)= x2_init;
    ...
    if ~isempty(x1i)
        ...
        hitmap(length(x2_init_vec)+1-x2_id,x1_id) = hit_time;
    else
        hit_vector_opt(zz) = NaN;
        hitmap((length(x2_init_vec)+1-x2_id),x1_id) = NaN;
    end 
...
ylabels = num2str(flip(x2_init_vec'));
