...
x1_lim = Def_lim/(1-Def_lim)*alpha_5;
hit_time = 0;
switch_time = 0;
singular_time = 0;
...
for ii=2:horizon
    ...
    b = [fert_rate;0]; 
    X1_temp = (A(1,:) * X(ii-1,:)' + f_nonlin(1) + b(1))'; % update state
    if((X1_temp<=x1_lim)) % && singular_time==0) % && singular_time==0
       adj_fert_rate=fert_rate+x1_lim-X1_temp; 
       if (adj_fert_rate <= u_min); adj_fert_rate = u_min; end
       if (adj_fert_rate >= u_max); adj_fert_rate = u_max; end
       b = [adj_fert_rate; 0]; 
    end
    fert_traj(ii) = b(1);
